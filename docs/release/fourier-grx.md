...
|----------|------|------|----------|----------|
| 2026-06-30 | **4.4.16** | [⬇ 下载](https://fourier-grx-1302548221.cos.ap-shanghai.myqcloud.com/grx/fourier-grx-4.4.16-linux-arm64-cpu-m4l-blaze.deb) | [详情](#4416) | ✅ 支持中 |
| 2026-06-30 | ~~4.4.15~~ | — | [详情](#4415) | ⚠️ 已撤销 |
...
- **`fourier-grx update` 次版本/主版本探测遗漏**：原逻辑对次版本只探测 `x.(y+i).0`，若实际首发版本为 `x.(y+i).1`（如 `4.5.1` 而无 `4.5.0`）则完全找不到。现改为对每个新次版本同样扫描 patch `0..10`（连续 3 次 404 后停止），主版本亦同步修正，扫描 minor `0..2` 下各 patch `0..10`

---

### ~~4.4.15~~ *(已撤销)*

> ⚠️ 仅修复了 patch 探测阈值，次版本/主版本探测逻辑仍有遗漏，已在 4.4.16 完整修复。
...
- **`fourier-grx update` 无法发现跨越撤销版本的新版本**：`_fourier_grx_do_update` 在探测可用补丁版本时，连续 404 的中止阈值由 3 提高到 5。若连续多个补丁版本被撤销（未上传至 COS），探测不再提前退出，最多可跨越 4 个空缺版本找到实际可用版本

---

### ~~4.4.14~~ *(已撤销)*

> ⚠️ `deb_run.sh` 中 `fourier-grx update` 探测逻辑未随本次改动一起更新，已在 4.4.15 修复。
