...
| 接口参数    | 类型      | 默认值 | 参数范围            | 描述                  |
|---------|---------|-----|-----------------|---------------------|
| 左大腿调节长度 | `float` | 0.0 | [0.0, 0.095] | 机器人的大腿调节段的长度，单位为 m。 |
| 左小腿调节长度 | `float` | 0.0 | [0.0, 0.095] | 机器人的小腿调节段的长度，单位为 m。 |
| 右大腿调节长度 | `float` | 0.0 | [0.0, 0.095] | 机器人的大腿调节段的长度，单位为 m。 |
| 右小腿调节长度 | `float` | 0.0 | [0.0, 0.095] | 机器人的小腿调节段的长度，单位为 m。 |
...
- `fourier-grx` v4.4.7 版本：大腿/小腿调节长度上限由 `0.090 m` 调整为 `0.095 m`，参数范围更新为 `[0.0, 0.095]`。
