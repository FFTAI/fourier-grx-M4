...
| 助力触发力上限 | `float` | 2.0   | [-∞, +∞]      | 自动助力模式中用于触发**增加**助力系数的力矩阈值，单位为 Nm。当实测关节力矩超过该值时，助力系数自动增加 0.01。 |
| 助力触发力下限 | `float` | 1.0   | [-∞, +∞]      | 自动助力模式中用于触发**减少**助力系数的力矩阈值，单位为 Nm。当实测关节力矩低于该值时，助力系数自动减少 0.01。 |
...
> | 触发条件 | 效果 |
> |--------|------|
> | 实测力矩 > 助力触发力上限 | 助力系数 += 0.01（患者主动用力 → 增加助力） |
> | 实测力矩 < 助力触发力下限 | 助力系数 -= 0.01（患者出力不足 → 减少助力） |
> | 其他情况 | 助力系数不变 |
