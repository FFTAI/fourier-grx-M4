...
| 助力触发力上限 | `float` | 2.0   | [-∞, +∞]      | 自动助力模式中用于触发**增加**助力系数的力矩阈值，单位为 Nm。当实测关节力矩超过该值时，助力系数自动增加 0.02。 |
| 助力触发力下限 | `float` | 1.0   | [-∞, +∞]      | 自动助力模式中用于触发**减少**助力系数的力矩阈值，单位为 Nm。当实测关节力矩低于该值时，助力系数自动减少 0.001。 |
| 自动助力模式标志位 | `bool`  | false | (true, false) | 是否启用自动助力模式。启用后系统根据关节实测力矩与参考轨迹的偏差每控制周期（20 ms）自动调整助力系数（患者用力时 +0.02，患者出力不足时 -0.001），无需手动设定；助力系数始终限制在 [0.0, 1.0] 范围内。                 |
...
> | 触发条件 | 效果 |
> |--------|------|
> | 实测力矩 > 助力触发力上限 | 助力系数 += 0.02（患者主动用力 → 增加助力） |
