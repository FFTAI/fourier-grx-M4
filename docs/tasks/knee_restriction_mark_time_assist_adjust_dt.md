...
| 助力触发力上限 | `float` | 2.0   | [-∞, +∞]      | 自动助力模式中用于触发**增加**助力系数的力矩阈值，单位为 Nm。当实测关节力矩超过该值时，助力系数自动增加 0.04。 |
| 助力触发力下限 | `float` | 1.0   | [-∞, +∞]      | 自动助力模式中用于触发**减少**助力系数的力矩阈值，单位为 Nm。当实测关节力矩低于该值时，助力系数自动减少 0.001。 |
| 自动助力模式标志位 | `bool`  | false | (true, false) | 是否启用自动助力模式。启用后系统根据关节实测力矩与参考轨迹的偏差每控制周期（20 ms）自动调整助力系数（患者用力时 +0.04，患者出力不足时 -0.001），无需手动设定；助力系数始终限制在 [0.0, 1.0] 范围内。                 |
...
> **助力触发力阈值说明**：
>
> 这两个参数直接作为绝对力矩阈值使用，控制自动助力模式下助力系数的增减。
>
> > **注意**：设计上曾考虑将重力补偿力矩 G[i] 加入触发阈值（即阈值 = G[i] + 偏移量），以使触发条件随姿态自适应。
> > 但经实际实验测试发现，引入 G[i] 项反而容易导致系统不稳定，因此暂时将 G[i] 系数置为 0，
> > 直接使用配置值作为绝对阈值。
>
> | 触发条件 | 效果 |
> |--------|------|
> | 实测力矩 > 助力触发力上限 | 助力系数 += 0.04（患者主动用力 → 增加助力） |
> | 实测力矩 < 助力触发力下限 | 助力系数 -= 0.001（患者出力不足 → 减少助力） |
> | 其他情况 | 助力系数不变 |
...
| 助力触发力上限 | `grx.virtual_panel_command_param_4`                                                      |
| 助力触发力下限 | `grx.virtual_panel_command_param_5`                                                      |
...
- 新增 `助力触发力上限` / `助力触发力下限` 参数，用于自动助力模式中可配置的绝对力矩触发阈值。经实际实验测试，将重力补偿力矩 G[i] 加入阈值公式反而容易导致系统不稳定，因此 G[i] 项系数已置为 0，直接使用配置值作为绝对阈值。
- `fourier-grx` v4.4.25 版本调整助力触发下限的自动调整幅度：由 -0.01 调整为 -0.001，避免原地踏步场景下助力比例回落过快导致的助力不稳定；触发上限的增量 +0.02（v4.4.24 起）保持不变。
- `fourier-grx` v4.4.30 版本将助力触发上限的自动调整增量由 +0.02 调整为 +0.04，进一步加快用力时的助力响应速度。