...
- 支持力矩保护功能：当任意关节力矩超过设定阈值时，任务将立即退出并切换至空闲任务（`TASK_IDLE`），同时置位过载标志，需重新下发本任务才能继续。

任务参数：

| 接口参数     | 类型      | 默认值   | 参数范围         | 描述                                   |
|----------|---------|-------|--------------|--------------------------------------|
| 启用力矩保护开关 | `bool`  | false | (true, false) | 是否启用力矩保护功能。启用后，当任意关节力矩超过"保护力矩限值"时，任务将立即退出并切换至空闲任务（`TASK_IDLE`），同时置位过载标志，需重新下发本任务才能继续。                  |
