...
Download the Fourier-GRX-M4 SDK installation package from the [Firmware Updates](/fourier-grx-M4/docs/en/update) page.
After downloading, run the following commands to complete the installation:

> ⚠️ **Deprecated**: The following is the installation method for the legacy `fourier-m4` software, which is no longer actively maintained.

```bash
# Fourier-M4 installation (legacy)
# Install the fourier-m4-xxx.deb file. After installation, the `fourier-m4` CLI tool will be available.
...
```bash
# Fourier-GRX installation (current)
# Install the fourier-grx-xxx.deb file. After installation, the `fourier-grx` CLI tool will be available.
sudo dpkg -i fourier-grx-xxx.deb  # xxx is the specific version number

# Install the full fourier-grx content
fourier-grx install
```

> ℹ️ **Note**:
>
> - If the installation fails due to network or other issues, simply re-run the installation command above.
> - A wired network connection is recommended.
> - If using a wired network, you must first configure the robot's network to DHCP mode so the device can access the internet.
> - The installation process will automatically configure the robot's wired network to use a static IP address so the program can operate normally afterward.

During installation, you will be prompted to enter the robot model. Configure it according to your actual robot. Currently supported models are:

> ⚠️ **Deprecated**: The robot models listed below are for the legacy `fourier-m4` software, which is no longer actively maintained.

`Fourier-M4 (Legacy)` robot model configuration options:
...
`Fourier-GRX (Current)` robot model configuration options:
...
>
> - **M4LT2**: Latest model, equipped with **2.5-generation FSA actuators**.
> - **M4LT1**: Equipped with **2.0-generation FSA actuators**.
> - **M4LP1**: Equipped with legacy **1.0-generation FSA actuators**, upgraded via software to be compatible with the 2.0-generation interface.

> ℹ️ **Run Mode Notes**
>
> - The **Developer Mode** (`sdk`) shown in the table above is for secondary development only, intended for scenarios where you are building custom applications via the SDK interfaces.
> - For **production/released robots**, the run mode should be configured to **Release Mode** (`release`) so the robot can be controlled via the upper-level application.
> - Use `fourier-grx config` to change the run mode. See the [Run Modes](/fourier-grx-M4/docs/en/reference/run_type) reference page for details.

If configured incorrectly, run `fourier-grx config` to reconfigure. Current configuration can be viewed with `fourier-grx list`.

> ℹ️ **Note**
>
> `fourier-m4` is the predecessor to `fourier-grx`, referred to as the "legacy version", and is no longer actively maintained. If your device supports the current `fourier-grx`, it is recommended to use `fourier-grx` for installation and operation.
