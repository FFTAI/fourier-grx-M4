...
|----|--------|---------------|----------------------------------------|------|------|
| 1  | 发布模式   | release       | 适用于机器人出厂使用，默认使用 **上位机** 进行机器人控制         | ✅    | ❌    |
...
> ℹ️ **说明**：
>
> 关于任务和模块的具体描述，以及更多任务功能信息，请参阅 [参考指南 User 接口](/fourier-grx-M4/docs/reference/user) 的 `task/client 接口协议 (指令信息)` 章节的内容。

目前 Fourier M4L 在调试模式下，可通过手柄调用的内部主要功能包括：

| 功能名称       | 任务编号 | 是否包含模块 | 说明           | 手柄快捷键              |
|------------|------|----------|----------------|--------------------|
| 执行器失能（软急停） | 36   | 否        | 机器人全身执行器失能任务   | Logo<br/>(XBOX 键位) |
| 清除故障       | 34   | 否        | 清除机器人全关节报警     | A<br/>(XBOX 键位)    |
| 站立姿态控制     | 4020 | 否        | 机器人缓慢移动各关节恢复到站立姿态 | X<br/>(XBOX 键位)    |
| 被动前向行走     | 4111 | 否        | 被动前向行走任务       | Y<br/>(XBOX 键位)    |
...
- 开发接口：[参考指南](/fourier-grx-M4/docs/reference)
